...
function displacement_cost = xx_msf_large_motion(pre_phase_scales, cur_phase_scales, ...
                                                 pre_gray_scales, cur_gray_scales, ...
                                                 pre_roi, cur_roi, orient_angle, iterations, method)
%     iterations = 10;
            ...
            for y = -half_patch_size : half_patch_size
               for x = -half_patch_size : half_patch_size
                   ...
                   if (cur_row_int+y+1 > s_size(1) || cur_row_int+y-1 < 1)
                       J_phase_pixel(1) = cur_gray_scales(min(cur_row_int+y+1, s_size(1)), cur_col_int+x)...
                                          - cur_gray_scales(max(cur_row_int+y-1, 1), cur_col_int+x);
                   else
                       J_phase_pixel(1) = 0.5 * (cur_gray_scales(cur_row_int+y+1, cur_col_int+x) - ...
                                                 cur_gray_scales(cur_row_int+y-1, cur_col_int+x));
                   end
                   if (cur_col_int+x+1 > s_size(2) || cur_col_int+x-1 < 1)
                       J_phase_pixel(2) = cur_gray_scales(cur_row_int+y, min(cur_col_int+x+1, s_size(2)))...
                                          - cur_gray_scales(cur_row_int+y, max(cur_col_int+x-1, 1));
                   else
                       J_phase_pixel(2) = 0.5 * (cur_gray_scales(cur_row_int+y, cur_col_int+x+1) - ...
                                                 cur_gray_scales(cur_row_int+y, cur_col_int+x-1));
                   end
                   J_pixel_delta = [sin(orient_angle); cos(orient_angle)];
                   error = msf_get_pixel_value(pre_gray_scales, row, col) - ...
                           msf_get_pixel_value(cur_gray_scales, cur_row, cur_col);
                           
                   J = -1.0 * J_phase_pixel' * J_pixel_delta;
                   % 灰度法中防止J_phase_pixel为0导致矩阵求解出Nan
                   if (J == 0)
                       continue;
                   end
                   hessian = hessian + J * J';
                   ...
                   tmp_cost_copy = tmp_cost_copy + (error / (J)) * (error / (J));
                   num_count = num_count + 1;
%                    cost_copy(p) = cost_copy(p) + (error / (J)) * (error / (J));
%                    cost_data = [cost_data, error / J];
            ...
            if (num_good > 0)
                H = H + hessian;
                b = b + bias;
                cost_copy(p) =  tmp_cost_copy / num_count;
            end
        end
