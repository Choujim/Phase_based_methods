...
function displacement_cost = x_msf_large_motion(pre_re_scales, cur_re_scales, pre_roi, cur_roi, orient_angle, iterations, method)
%     iterations = 10;
    plane_constraint_ratio = 0.3;
    s_size = size(pre_roi);
    roi_index = find(logical(pre_roi));
    
    pre_phase_scales = angle(pre_re_scales);
    cur_phase_scales = angle(cur_re_scales);
...
%     fprintf("t_cur_roi: %f\n", t_cur_roi);
    ...
    t_init = t_init + dot((t_cur_roi - t_pre_roi), t_unit); 
%     fprintf("t_init: %f\n", t_init);
...
%                                           - cur_phase_scales(max(cur_row_int+y-1, 1), cur_col_int+x);
                       delta_im = (imag(cur_re_scales(min(cur_row_int+y+1, s_size(1)), cur_col_int+x)) - ...
                                         imag(cur_re_scales(max(cur_row_int+y-1, 1), cur_col_int+x)));
                       delta_re = (real(cur_re_scales(min(cur_row_int+y+1, s_size(1)), cur_col_int+x)) - ...
                                         real(cur_re_scales(max(cur_row_int+y-1, 1), cur_col_int+x)));
                       J_phase_pixel(1) = (real(cur_re_scales(cur_row_int+y, cur_col_int+x)) * delta_im - ...
                                           imag(cur_re_scales(cur_row_int+y, cur_col_int+x) * delta_re)) / (abs(cur_re_scales(cur_row_int+y, cur_col_int+x))^2);
                   else
%                        J_phase_pixel(1) = 0.5 * (cur_phase_scales(cur_row_int+y+1, cur_col_int+x) - ...
%                                                  cur_phase_scales(cur_row_int+y-1, cur_col_int+x));
                       delta_im = 0.5 * (imag(cur_re_scales(cur_row_int+y+1, cur_col_int+x)) - ...
                                         imag(cur_re_scales(cur_row_int+y-1, cur_col_int+x)));
                       delta_re = 0.5 * (real(cur_re_scales(cur_row_int+y+1, cur_col_int+x)) - ...
                                         real(cur_re_scales(cur_row_int+y-1, cur_col_int+x)));
                       J_phase_pixel(1) = (real(cur_re_scales(cur_row_int+y, cur_col_int+x)) * delta_im - ...
                                           imag(cur_re_scales(cur_row_int+y, cur_col_int+x) * delta_re)) / (abs(cur_re_scales(cur_row_int+y, cur_col_int+x))^2);              
                   end
...
%                                           - cur_phase_scales(cur_row_int+y, max(cur_col_int+x-1, 1));
                       delta_im = (imag(cur_re_scales(cur_row_int+y, min(cur_col_int+x+1, s_size(2)))) - ...
                                         imag(cur_re_scales(cur_row_int+y, max(cur_col_int+x-1, 1))));
                       delta_re = (real(cur_re_scales(cur_row_int+y, min(cur_col_int+x+1, s_size(2)))) - ...
                                         real(cur_re_scales(cur_row_int+y, max(cur_col_int+x-1, 1))));
                       J_phase_pixel(2) = (real(cur_re_scales(cur_row_int+y, cur_col_int+x)) * delta_im - ...
                                           imag(cur_re_scales(cur_row_int+y, cur_col_int+x) * delta_re)) / (abs(cur_re_scales(cur_row_int+y, cur_col_int+x))^2);
                   else
%                        J_phase_pixel(2) = 0.5 * (cur_phase_scales(cur_row_int+y, cur_col_int+x+1) - ...
%                                                  cur_phase_scales(cur_row_int+y, cur_col_int+x-1));
                       delta_im = 0.5 * (imag(cur_re_scales(cur_row_int+y, cur_col_int+x+1)) - ...
                                         imag(cur_re_scales(cur_row_int+y, cur_col_int+x-1)));
                       delta_re = 0.5 * (real(cur_re_scales(cur_row_int+y, cur_col_int+x+1)) - ...
                                         real(cur_re_scales(cur_row_int+y, cur_col_int+x-1)));
                       J_phase_pixel(2) = (real(cur_re_scales(cur_row_int+y, cur_col_int+x)) * delta_im - ...
                                           imag(cur_re_scales(cur_row_int+y, cur_col_int+x) * delta_re)) / (abs(cur_re_scales(cur_row_int+y, cur_col_int+x))^2); 
                   end
                   J_pixel_delta = [sin(orient_angle); cos(orient_angle)];
                   
%                    v_pre = msf_get_pixel_value(pre_re_scales, row, col);
%                    v_pre = v_pre ./ abs(v_pre);
%                    v_cur = msf_get_pixel_value(cur_re_scales, cur_row, cur_col);
%                    v_cur = v_cur ./ abs(v_cur);
%                    v_mid = v_pre + v_cur;
%                    v_delta = v_pre - v_cur;
%                    error_sign = sign(real(v_mid) * imag(v_delta) - imag(v_mid) * real(v_delta)); % sign 为正代表相位差为正，反之为负
%                    delta_theta = acos(real(real(v_pre) * real(v_cur) + imag(v_pre) * imag(v_cur)) / (abs(v_pre)^2 * abs(v_cur)^2));
%                    error = real(error_sign * delta_theta);
%                    delta_theta = (1.0 - real(v_pre) * real(v_cur) - imag(v_pre) * imag(v_cur));
%                    error = real(delta_theta);

%                    error = mod((pi + msf_get_pixel_value(pre_phase_scales, row, col) - ...
%                            msf_get_pixel_value(cur_phase_scales, cur_row, cur_col)), 2*pi)-pi;
                       
                   error = msf_get_pixel_value(pre_phase_scales, row, col) - ...
                           msf_get_pixel_value(cur_phase_scales, cur_row, cur_col);

%                    J = -1.0 * (real(v_cur) * imag(v_pre) - imag(v_cur) * real(v_pre)) * J_phase_pixel' * J_pixel_delta;
