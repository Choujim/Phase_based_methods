...
function [pyr,pind] = buildSCFpyrLevs(lodft,log_rad,Xrcos,Yrcos,angle_map,ht,nbands);
...
%
  ...
  for b = 1:nbands
    anglemask = pointOp(angle_map, Ycosn, Xcosn(1)+pi*(b-1)/nbands, Xcosn(2)-Xcosn(1));
...
%    bands(:,b) = real(band(:));
  ...
  [npyr,nind] = buildSCFpyrLevs(lodft, log_rad, Xrcos, Yrcos, angle_map, ht-1, nbands);
