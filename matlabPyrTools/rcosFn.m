...
X    = pi * [-sz-1:1] / (2*sz); % 生成[-pi/2, 0]内的离散点

Y = values(1) + (values(2)-values(1)) * cos(X).^2; % 指定偏移和放大倍数

%    Make sure end values are repeated, for extrapolation...
Y(1) = Y(2);
Y(sz+3) = Y(sz+2);

X = position + (2*width/pi) * (X + pi/4); % 将X映射回width宽度内的离散点
% position + (-pi/4, pi/4) * 2*width/pi = (-width/2, width/2) + position
