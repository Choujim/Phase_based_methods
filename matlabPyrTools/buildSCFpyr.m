...
elseif ((order > 15)  | (order < 0))
  ...
  order = min(max(order,0),15); % reset invalid order as 0 or 15
else
  order = round(order);
end
nbands = order+1;
...
if (mod((nbands),2) == 0)
  harmonics = [0:(nbands/2)-1]'*2 + 1; % ' Îª×ªÖÃ
else
...
[pyr,pind] = buildSCFpyrLevs(lo0dft, log_rad, Xrcos, Yrcos, angle_map, ht, nbands);
