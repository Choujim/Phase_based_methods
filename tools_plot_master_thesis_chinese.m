...
legend(' cos(2\itx\rm)', ' cos(2(\itx\rm-pi/6))', ' cos(2\itx\rm-pi/3)');
legend('boxoff')
...
text(center_1+5, y_line_1(center_1+2), ['\itx\rm=40'] ,'FontSize',10,'Fontname','Times New Roman');hold on;
plot(center_1, y_line_1(center_1+2), 'ob', 'MarkerSize', 4); hold on;
text(center_1-3, y_line_1(center_1+2), ['\itx\rm=38'], 'HorizontalAlignment','right', 'FontSize',10,'Fontname','Times New Roman');hold on;
% lgd = legend('Shifted', 'Original');
lgd = legend('平移操作', '原始相位');
...
text(center_1-3,res(center_1), ['\itx\rm=38'], 'HorizontalAlignment','right' ,'FontSize',10,'Fontname','Times New Roman');hold on;
% lgd = legend('Phase shifted', 'Original');
lgd = legend('相移操作', '原始相位');
...
ylabel('局部相位(rad)',  'Fontname', '宋体')
xlabel('\itx\rm(pixel)');
yticks(-4 : 2 : 4);
% lgd = legend('Original Phase', 'Shifted Phase ');
lgd = legend( '原始相位', '相移操作 ');
% legend('Location','northeast');
lgd.FontSize = 8;
...
ylabel(ax_1, '局部相位(rad)',  'Fontname', '宋体');
xlim(ax_1, [0 121]);
xlabel(ax_1, '\itx\rm(pixel)');
...
lgd = legend(ax_1, '原始', '有噪声', '左移1像素');
% legend(ax_1,'Location','southeast');
legend(ax_1, 'boxoff');
lgd.FontSize = 8;
lgd.Position = [0.68, 0.28, 0.1, 0.1];
...
legend('boxoff');
lgd.FontSize = 8;
lgd.Position = [0.66, 0.75, 0.1, 0.1];
...
ylabel('空间相位梯度(rad/pixel)',  'Fontname', '宋体');
xlim([0 121]);
yticks(-pi : pi/2 : pi/2);
yticklabels({'-pi' '-pi/2' '0' 'pi/2' 'pi'});
xlabel('\itx\rm(pixel)');
% legend('Wrapped', 'True');
lgd = legend('相位包裹的梯度', '真实梯度');
legend('Location','northeast');
legend('boxoff');
% lgd.FontSize = 10;
lgd.Position = [0.65, 0.78, 0.1, 0.1];
set(gca,'FontSize',10,'Fontname','Times New Roman');
...
% ylabel('Phase Gradient(rad/pixel)');
ylabel('相位梯度(rad/pixel)',  'Fontname', '宋体');
set(gca,'FontSize',10,'Fontname','Times New Roman');
yticks(0 : 0.4 : 1.2);
% yticklabels({'-pi' '-pi/2' '0' 'pi/2' 'pi'});
xlim([0 121]);
...
lgd = legend('直接差商', '用微分式差商');
% legend('Location','northwest');
legend('boxoff');
lgd.Interpreter = 'none';
lgd.FontSize = 8;
lgd.Position = [0.27, 0.75, 0.1, 0.1];
...
ylabel('\omega_{\ity\rm}(pixel^{-1})', 'position', [-10 300]);
xlabel('\omega_{\itx\rm}(pixel^{-1})', 'position', [300 600]);
ax = gca;
% ax.YAxis.Label.Position(1) = -0.1;
% ax.XAxis.Label.Position(1) = 0.1;
...
ylabel('幅值(pixel)', 'Fontname', 'Times New Roman');
% xlabel('Time(s)');
xlabel('时间(s)', 'Fontname', 'Times New Roman');
xticks(0 : 60 : 300);
xticklabels({'0' '2' '4' '6' '8' '10'});
% lgd = legend('True', 'scale 1', 'scale 2', 'scale 3');
lgd = legend('真实值', '尺度1', '尺度2', '尺度3');
% legend('Location','northeast');
legend('boxoff');
legend('Orientation','horizontal');
lgd.FontSize = 8;
lgd.Position = [0.44, 0.92, 0.07, 0.1];
...
% ylabel('Error(pixels)');
ylabel('误差(pixel)', 'Fontname', 'Times New Roman');
% xlabel('Time(s)');
xlabel('时间(s)', 'Fontname', 'Times New Roman');
xticks(0 : 60 : 300);
xticklabels({'0' '2' '4' '6' '8' '10'});
% lgd = legend('scale 1', 'scale 2', 'scale 3');
lgd = legend('尺度1', '尺度2', '尺度3');
% legend('Location','northeast');
legend('boxoff');
% legend('Orientation','horizontal');
lgd.FontSize = 8;
...
ylim(y_lim_1(video_idx,:));
% ylabel('MAE (pixel)');
ylabel('MAE (pixel)', 'Fontname', 'Times New Roman');
yticks(y_lim_1(video_idx,1) : y_step_1(video_idx) : y_lim_1(video_idx,2));
% xlabel('Scale');
xlabel('尺度', 'Fontname', 'Times New Roman');
% xticks(0 : 60 : 300);
% xticklabels({'0' '2' '4' '6' '8' '10'});
% lgd = legend('\sigma_{\itaf\rm} = \pi', '\sigma_{\itaf\rm} = \pi/2', '\sigma_{\itaf\rm} = \pi/4', '\sigma_{\itaf\rm} = \pi/8', '\sigma_{\itaf\rm} = \pi/16', '\sigma_{\itaf\rm} = \pi/32');
lgd = legend('$\sigma_{af}=\pi$', '$\sigma_{af}=\pi/2$', '$\sigma_{af}=\pi/4$', '$\sigma_{af}=\pi/8$', '$\sigma_{af}=\pi/16$', '$\sigma_{af}=\pi/32$');
legend('Location','northeast');
legend('boxoff');
% legend('Orientation','horizontal');
lgd.FontSize = 8;
...
lgd = legend('CSP 滤波器', 'Gabor 滤波器');
legend('Location','northeast');
legend('boxoff');
% legend('Orientation','horizontal');
lgd.FontSize = 8;
% lgd.Interpreter = 'latex';
lgd.Position = [0.65, 0.79, 0.1, 0.1];
...
ylabel('边缘方向(deg)', 'Fontname', 'Times New Roman', 'position', [-20 68]);
...
title('时间(frame)', 'position', [151 60]);
...
ylabel('边缘方向(deg)', 'Fontname', 'Times New Roman', 'position', [-20 14]);
...
ylim([0 dim(1)+1]);
% ylabel('Height (pixel)');
ylabel('高度(pixel)',  'Fontname', '宋体');
set(gca,'FontSize',10,'Fontname','Times New Roman');
xlim([0 dim(2)+1]);
ax = gca;
ax.XAxisLocation = 'top';
% xlabel('Width (pixel)');
xlabel('宽度(pixel)',  'Fontname', '宋体');
set(gca,'FontSize',10,'Fontname','Times New Roman');
...
ylabel(ax_1, '幅值(pixel)', 'Fontname', 'Times New Roman');
% xlabel(ax_1, 'Time(s)');
xlabel(ax_1, '时间(s)', 'Fontname', 'Times New Roman');
...
lgd.FontSize = 8;
% lgd.Interpreter = 'latex';
lgd.Position = [0.73, 0.78, 0.1, 0.1];
...
ylabel(ax_1, '误差(pixel)', 'Fontname', 'Times New Roman');
% xlabel(ax_1, 'Time(s)');
xlabel(ax_1, '时间(s)', 'Fontname', 'Times New Roman');
xticks(ax_1, 0 : 60 : 300);
xticklabels(ax_1, {'0' '2' '4' '6' '8' '10'});
...
lgd = legend(ax_1, '真实值', '相邻帧法', '参考帧法', '提出的方法');
legend(ax_1, 'Location','southeast');
legend(ax_1, 'boxoff');
% legend(ax_1, 'Orientation','horizontal');
lgd.FontSize = 8;
...
lgd = legend(ax_1, '相邻帧法', '参考帧法', '提出的方法');
legend(ax_1, 'Location','southeast');
legend(ax_1, 'boxoff');
% legend('Orientation','horizontal');
lgd.FontSize = 8;
...
legend(ax_1, 'boxoff');
legend(ax_1, 'Orientation','horizontal');
lgd.Position = [0.76, 0.82, 0.1, 0.1];
...
lgd = legend(ax_1, '真实值', '噪声1', '噪声2', '噪声3');
legend(ax_1, 'Location','northeast');
legend(ax_1, 'boxoff');
legend(ax_1, 'Orientation','horizontal');
lgd.FontSize = 8;
...
ylim(y_lim_1(video_idx,:));
% ylabel('MAE (mm)');
ylabel('MAE(mm)', 'Fontname', 'Times New Roman');
yticks(y_lim_1(video_idx,1) : y_step_1(video_idx) : y_lim_1(video_idx,2));
% xlabel('Distance (m)');
xlabel('距离(m)', 'Fontname', 'Times New Roman');
xticks(1 : 1 : 3);
xticklabels({'25' '50' '100'});
% lgd = legend('noise 1', 'noise 2', 'noise 3');
lgd = legend('噪声1', '噪声2', '噪声3');
legend('Location','northwest');
legend('boxoff');
% legend('Orientation','horizontal');
lgd.FontSize = 8;
...
ylim(y_lim_1(video_idx,:));
% ylabel('PSD (dB/Hz)');
ylabel('PSD(dB/Hz)', 'Fontname', 'Times New Roman');
yticks(y_lim_1(video_idx,1) : y_step_1(video_idx) : y_lim_1(video_idx,2));
% xlabel('Frequency (Hz)');
xlabel('频率(Hz)', 'Fontname', 'Times New Roman');
xticks(0 : 3 : 15);
% xticklabels({'25' '50' '100'});
% lgd = legend('noise 1', 'noise 2', 'noise 3');
lgd = legend('噪声1', '噪声2', '噪声3');
...
ylabel('索力(kN)', 'Fontname', 'Times New Roman');
% xlabel('Time(h)');
xlabel('时间(h)', 'Fontname', 'Times New Roman');
...
set(gca,'FontSize',10,'Fontname','Times New Roman');
ylim([0, max(data2show_2)+50]);
% yticks(y_lim_1(idx_cable,1) : y_step_1(idx_cable) : y_lim_1(idx_cable,2));
yticks([]);
% ylabel('Cable Force(kN)');
% xlabel('Time(point)');
xlabel('时间(point)', 'Fontname', 'Times New Roman');
...
ylabel('索力(kN)', 'Fontname', 'Times New Roman');;
% xlabel('Time(day)');
xlabel('时间(天)', 'Fontname', 'Times New Roman');
